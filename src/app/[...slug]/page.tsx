...
import { getGlobalSettings, getLocationBySlug, getNearbyLocations, getServices } from '@/lib/data';
...
export default async function LocalPageDynamic({ params }: PageParams) {
  ...
  const settings = await getGlobalSettings();
  let localPage = await prisma.localPage.findUnique({
    where: { slug: slugStr, status: 'published' },
  });

  if (!localPage && slug.length === 1) {
    const matchedKeyword = PRIMARY_KEYWORDS.find((keyword) =>
      slug[0].startsWith(`${keyword.slug}-`),
    );
    if (matchedKeyword) {
      const legacySlug = `/${matchedKeyword.slug}/${slug[0].replace(
        `${matchedKeyword.slug}-`,
        '',
      )}`;
      localPage = await prisma.localPage.findUnique({
        where: { slug: legacySlug, status: 'published' },
      });
    }
  }
  ...
  if (!localPage) {
    if (slug.length === 1) {
      const location = await getLocationBySlug(slug[0]);
      if (!location) {
        notFound();
      }
        ...
        <div className="space-y-10">
          ...
          <section className="panel space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="space-y-3 text-sm text-slate-700">
                ...
                {location.localStatsOrRegulationNotes && <p>{location.localStatsOrRegulationNotes}</p>}
          ...
          <section className="panel grid gap-6 lg:grid-cols-[1.4fr,1fr]">
            ...
            <div className="surface p-4">
              ...
              <p className="text-sm text-slate-600">
                Crews follow NFPA-96 and local fire codes for {location.state}. Every visit includes
